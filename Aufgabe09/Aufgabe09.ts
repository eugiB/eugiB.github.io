namespace Aufgabe9 {
    ...
    function init(_event: Event): void {
        ...
        for (let i: number = 0; i < 8; i++) {
            let _x: number = Math.floor(Math.random() * crc2.canvas.width);
            let _y: number = Math.floor(Math.random() * (70 - 250) + crc2.canvas.height);
            drawTree(_x, _y);
        }
    ...
    function drawSnow(a: number, b: number): void {
        crc2.beginPath();
